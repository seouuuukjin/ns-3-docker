...
#ifndef UDP_RELIABLE_ECHO_CLIENT_H
#define UDP_RELIABLE_ECHO_CLIENT_H
 ...
 */
class UdpReliableEchoClient : public Application 
{
public:
  /**
  ...
  UdpReliableEchoClient ();

  virtual ~UdpReliableEchoClient ();
...
private:
  ...
  //TODO:
  SeqTsHeader seqTS;
  uint32_t m_waitingSeqNumber;
  std::queue<uint32_t> PendingTransmissonQueue;
  uint32_t m_localSeqNumber;

  uint32_t m_num_of_dropped_packet;
  uint32_t m_num_of_received_retransPacket;

  uint32_t GetLocalSeqNumber(); // Returns Seq Number that have to be sent this order
...
#endif /* UDP_RELIABLE_ECHO_CLIENT_H */
