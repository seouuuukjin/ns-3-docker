 ...
 */
#ifndef UDP_RELIABLE_ECHO_HELPER_H
#define UDP_RELIABLE_ECHO_HELPER_H
...
/**
 * \ingroup UdpReliableEcho
 * \brief Create a server application which waits for input UDP packets
 *        and sends them back to the original sender.
 */
class UdpReliableEchoServerHelper
{
public:
  /**
   * Create UdpReliableEchoServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  UdpReliableEchoServerHelper (uint16_t port);
  ...
  /**
   * Create a UdpReliableEchoServerApplication on the specified Node.
   *
  ...
  /**
   * Create a UdpReliableEchoServerApplication on specified node
   *
...
private:
  /**
   * Install an ns3::UdpReliableEchoServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an UdpReliableEchoServer will be installed.
 ...
 */
class UdpReliableEchoClientHelper
{
public:
  /**
   * Create UdpReliableEchoClientHelper which will make life easier for people trying
   ...
   */
  UdpReliableEchoClientHelper (Address ip, uint16_t port);
  /**
   * Create UdpReliableEchoClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   ...
   */
  UdpReliableEchoClientHelper (Address addr);
  ...
  /**
   * Given a pointer to a UdpReliableEchoClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be UdpReliableEchoClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the UdpReliableEchoClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the UdpReliableEchoClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::UdpReliableEchoClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an UdpReliableEchoClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
...
#endif /* UDP_RELIABLE_ECHO_HELPER_H */
