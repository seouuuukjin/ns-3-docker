...
NS_LOG_COMPONENT_DEFINE ("week6_skeleton");
...
{
  LogComponentEnable("week6_skeleton", LOG_LEVEL_ALL);

  //we are going to change this thing per condition by waf command and CMD Parser
...
/* ToDo: Install packet sinks to the destinations
  ...
  ApplicationContainer sinkAppTcp = packetSinkHelperTcp.Install(nRtrnDst.Get(1));
  ...
  ApplicationContainer sinkAppUdp = packetSinkHelperUdp.Install(nRtrnDst.Get(1));
//==========================================================================================
  ...
  onoffTcp.SetAttribute("DataRate", DataRateValue(500000)); // = 0.5Mb/s
...
/* ToDo: Connect the trace source and the trace sink
  ...
  Ptr<Socket> ns3TcpSocket = Socket::CreateSocket (nSrc1nRtr.Get(0), TcpSocketFactory::GetTypeId());
